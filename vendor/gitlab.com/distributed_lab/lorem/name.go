...
// RandomNameIter will ensure uniqueness of returned values.
// It's a caller responsibility to close the channel if needed.
	...
	go func() {
		defer func() {
			if rvr := recover(); rvr != nil {
				// depr, better ideas are welcome
				if fmt.Sprintf("%v", rvr) != "send on closed channel" {
					panic(rvr)
				}
			}
		}()
		...
		for {
			if try > 64 {
				// looks like we exhausted given namespace
				panic("no more names for you")
			}
			guess := RandomName()
			if _, ok := already[guess]; ok {
				try += 1
				continue
			}
			already[guess] = struct{}{}
			ch <- guess
		}
